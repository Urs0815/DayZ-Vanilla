		...
		if (player)
		{
			...
			if (hcv)
			{
				...
				if (car)
				{
					...
					m_VehicleGearCount = car.GetGearsCount();
...
*/
	...
	{
		SetLeftStatsVisibility(!m_HudHidePlayer && !m_HudHideUI && m_HudState || m_HudInventory);
	...
	{
		if (m_QuickbarHidePlayer && !itemChanged) // Don't update when it is hidden
			return;
